...
if __name__ == '__main__':
    ...
    end_time = time.time()
    print('串行总耗时：%.5f 秒' % float(end_time-start_time))

    '''使用线程池方式执行'''
    start_time = time.time()

    pool = Pool(3)
    # 将列表中的每一个元素作为参数传递给get_response方法
    response_list = pool.map(get_response, urls)
    chapter_list = pool.map(parse_page_text, response_list)
