...
# **********获取QQ的窗口句柄********** #
# qq = win32gui.FindWindow(None, "QQ")     # 参数1是类名，参数2是QQ软件的标题
# place = win32gui.GetWindowPlacement(qq)  # (0, 1, (-1, -1), (-1, -1), (1623, 6, 1914, 716))
# title = win32gui.GetWindowText(qq);    # 获取活动窗口标题
# clsname = win32gui.GetClassName(qq);   # 获取活动窗口类名
# 获取QQ窗口的焦点
# Mouse.mouse_click(place[4][0]+100, place[4][1]+100)
# **********获取QQ的窗口句柄********** #
...
# 打开QQ
def openQQ():
    os.popen("c:\\Users\\汪洋\\Desktop\\QQ.lnk")  # os.system方法 必须等该进程执行完后
    time.sleep(1.5)
    # 回车登录
    win32api.keybd_event(VK_CODE["enter"], 0, 0, 0)
    time.sleep(0.1)
    win32api.keybd_event(VK_CODE["enter"], 0, win32con.MOUSEEVENTF_LEFTUP, 0)
...
def control():
    while True:
        ...
        if content == "下一首":
            nextMusic()
            # 清屏方便重新获取指令
            Mouse.mouse_rclick(850, 350)
            Mouse.mouse_click(880, 650)
            time.sleep(1)
            continue
        elif content == "上一首":
            lastMusic()
